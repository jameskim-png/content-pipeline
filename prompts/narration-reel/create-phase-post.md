...
Talking Head와 근본적으로 다른 어셈블리:
- TH: 청크별 (비디오+오디오) 합성 -> concat
- NR: 무음 클립 concat -> 연속 나레이션 오버레이

## 1. 나레이션 트랙 결합

청크별 voice WAV를 하나의 연속 나레이션 트랙으로:
...
나레이션 릴스의 자막 타이밍은 voice_manifest 기반으로 계산:
...
# Use language-appropriate subtitle font
